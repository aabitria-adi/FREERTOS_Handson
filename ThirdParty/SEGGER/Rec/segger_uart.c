...
#define OFF_CRL             0x00        // GPIOx_MODER    (GPIO port mode register)
#define OFF_CRH             0x04
...
#define AFIO_BASE_ADDR      AFIO_BASE
...
#define UART_BASECLK        OS_FSYS / 1       // USART1 runs on APB2 clock
...
#define GPIO_CRL            *(volatile uint32_t*)(GPIO_BASE_ADDR + OFF_CRL)
...
void HIF_UART_Init(uint32_t Baudrate, UART_ON_TX_FUNC_P cbOnTx, UART_ON_RX_FUNC_P cbOnRx) {
  ...
  RCC_APB2ENR |= (1 <<  0);        // Enable AFIO clock in STM32F1xx
  v = GPIO_CRH;
  v &= ~((0x3 << 10) | (0x3 << 8) | (0x3 << 6) | (0x3 << 4));
  v |= ((0x1 << 10) | (0x0 << 8) | (0x2 << 6) | (0x3 << 4)); // AF will be used, now how to tell that USART1 is to use?
  GPIO_CRH = v;
...
//  }
  Div = UART_BASECLK / (16 * Baudrate);
  Rem = UART_BASECLK - (Div * (16 * Baudrate));
  Rem *= 16;
  Rem /= (16 * Baudrate);

  USART_BRR = (Div << 4) | (Rem << 0);
  //
